...
!*******************************************************************************
    ...
    SUBROUTINE interpolate_generic_field(field, x_point, y_point, z_point, mag)
    !Use for the field line quantities
    REAL(num):: x_point, y_point, z_point !Coordinates of point to interpolate
    REAL(num):: xp, yp, zp !Coordinates in the respective dimentions
    REAL(num), dimension(:,:,:):: field(0:nx+1,0:ny+1,0:nz+1)
    INTEGER:: xi, yi, zi !Cell indices in each dimension
    REAL(num):: xf, yf, zf !Distance 'up' each cell
    REAL(num):: mag
    ...
    xi = int(xp + 0.5_num); yi = int(yp + 0.5_num); zi = int(zp + 0.5_num)
    xf = xp + 0.5_num - xi; yf = yp + 0.5_num - yi; zf = zp + 0.5_num - zi

    mag = mag + field(xi,yi,zi)*(1.0_num-xf)*(1.0_num-yf)*(1.0_num-zf) + field(xi,yi,zi+1)*(1.0_num-xf)*(1.0_num-yf)*(zf)
    mag = mag + field(xi,yi+1,zi)*(1.0_num-xf)*(yf)*(1.0_num-zf)       + field(xi,yi+1,zi+1)*(1.0_num-xf)*(yf)*(zf)
    mag = mag + field(xi+1,yi,zi)*(xf)*(1.0_num-yf)*(1.0_num-zf)       + field(xi+1,yi,zi+1)*(xf)*(1.0_num-yf)*(zf)
    mag = mag + field(xi+1,yi+1,zi)*(xf)*(yf)*(1.0_num-zf)             + field(xi+1,yi+1,zi+1)*(xf)*(yf)*(zf)

    END SUBROUTINE interpolate_generic_field
