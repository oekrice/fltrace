...
def adjust_A_boundary_flux(a, b, xv, yv, zv):
    print('Adjusting vector potential for not-closed boundaries...')
    #Applies the boundary correction if there is nonzero flux through the boundaries
    a_adjust = a[:,:,:,:]
    #Run through slices of A and update
    for xi, x in enumerate(xv):
        print('%d %% done' % (100*xi/len(xv)), end='\r' )
        for yi, y in enumerate(yv):
            bsum = np.zeros(len(zv))
            #Side 1 -- x == x0:
            for yb, ybound in enumerate(yv):
                theta = np.arctan2(ybound - y, xv[0] - x)
                if theta < 0:   #Get branch cut right
                    theta = theta + 2.*np.pi
                bsum -= (0.5/np.pi)*theta*b[0,yb,:,1]*(yv[1] - yv[0])  #Sign here negative due to normal component being backwards
            #Side 2 -- x == x1:
            for yb, ybound in enumerate(yv):
                theta = np.arctan2(ybound - y, xv[-1] - x)
                if theta < 0:   #Get branch cut right
                    theta = theta + 2.*np.pi
                bsum += (0.5/np.pi)*theta*b[-1,yb,:,1]*(yv[1] - yv[0])
            #Side 3 -- y == y0:
            for xb, xbound in enumerate(xv):
                theta = np.arctan2(yv[0] - y, xbound - x)
                if theta < 0:   #Get branch cut right
                    theta = theta + 2.*np.pi
                bsum -= (0.5/np.pi)*theta*b[xb,0,:,1]*(xv[1] - xv[0])  #Sign here negative due to normal component being backwards
            #Side 4 -- y == y1:
            for xb, xbound in enumerate(xv):
                theta = np.arctan2(yv[-1] - y, xbound - x)
                if theta < 0:   #Get branch cut right
                    theta = theta + 2.*np.pi
                bsum += (0.5/np.pi)*theta*b[xb,-1,:,1]*(xv[1] - xv[0])  #Sign here negative due to normal component being backwards
            a_adjust[xi,yi,:,2] -= bsum
    return a_adjust
...
def twistDen(bField,curlField,tol):
    ...
    currDen = get_product_single(bField, curlField)
